...
void hcd_port_reset(uint8_t p) { (void)p; }
void hcd_port_reset_end(uint8_t p) { (void)p; }
bool hcd_port_connect_status(uint8_t p) { (void)p; return true; }
tusb_speed_t hcd_port_speed_get(uint8_t p) { (void)p; return TUSB_SPEED_HIGH; }
...
static void test_audio_ring(void) {
  ...
  audio_event(dev, XFER_RESULT_QUEUED, 0);
  assert(s->packet_count == 2 && first[0] == 0x11 && second[0] == 0x22);
  audio_event(dev, XFER_RESULT_SUCCESS, 24);
  assert(s->packet_count == 2 && s->packet_head == 1);
  uint8_t received[24];
  ...
  // Stop keeps the buffers owned until terminal events drain both slots.
  audioh_stream_stop_xfers(s);
  audio_event(dev, XFER_RESULT_QUEUED, 0);
  assert(s->packet_count == 2);
  audio_event(dev, XFER_RESULT_SUCCESS, 24);
  assert(s->packet_count == 1 && usbh_edpt_busy(1, 0x81));
...
static void test_playback_ring(void) {
  ...
  audio_event_ep(dev, 7, XFER_RESULT_QUEUED, 0);
  assert(second[0] == 0x22 && s->packet_count == 2);
  ...
  assert(dev->ep_pending[2][1] == 1 && s->packet_count == 2);
  audioh_stream_stop_xfers(s);
...
int main(void) {
  ...
  // A delayed capacity event does not create a third slot.
  assert(usbh_edpt_retire_event(dev, &e));
  assert(!usbh_edpt_claim(1, 0x81));
