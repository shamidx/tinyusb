...
#if CFG_TUH_EHCI_ISO_EP_MAX
  // Links remain fixed while an endpoint is open. Hardware-owned descriptors
  // never share cache lines with software endpoint state.
  iso_td_t iso_td[CFG_TUH_EHCI_ISO_EP_MAX][CFG_TUH_XFER_QUEUE_DEPTH][FRAMELIST_SIZE];
...
static uint32_t iso_now(void) {
  ...
  return ehci_data.iso_uframe - (ehci_data.regs->portsc_bm.nxp_port_speed == TUSB_SPEED_FULL ? 8u : 0u);
}

static uint32_t iso_earliest(uint32_t now) {
  uint32_t const threshold = ehci_data.cap_regs->hccparams_bm.iso_schedule_threshold;
...
static iso_td_t* iso_td(iso_ep_t const* ep, iso_req_t const* req) {
  ...
  return &ehci_data.iso_td[ep_index][req - ep->req][(req->scheduled_uframe >> 3) % FRAMELIST_SIZE];
}
...
static bool iso_ep_open(uint8_t rhport, uint8_t daddr, tusb_desc_endpoint_t const* desc) {
  ...
  for (size_t slot = 0; slot < CFG_TUH_XFER_QUEUE_DEPTH; slot++) {
    for (size_t frame = 0; frame < FRAMELIST_SIZE; frame++) {
      iso_td_t* td = &ehci_data.iso_td[ep_index][slot][frame];
      tu_memclr(td, sizeof(*td));
      td->itd.next = ehci_data.period_framelist[frame];
      hcd_dcache_clean(td, sizeof(*td));
      ehci_data.period_framelist[frame].address = (uint32_t) td |
        ((bus.speed == TUSB_SPEED_HIGH ? EHCI_QTYPE_ITD : EHCI_QTYPE_SITD) << 1);
    }
  }
  hcd_dcache_clean(ehci_data.period_framelist, sizeof(ehci_data.period_framelist));
...
static void iso_arm(iso_ep_t* ep, iso_req_t* req, uint32_t now) {
  ...
  ehci_link_t const next = td->itd.next;
  tu_memclr(td, sizeof(*td));
  td->itd.next = next;
  ...
  if (ep->speed == TUSB_SPEED_HIGH) {
    ...
    itd->BufferPointer[0] = page | ep->daddr | (tu_edpt_number(ep->ep_addr) << 8);
    itd->BufferPointer[1] = (page + 4096) | ep->packet_size | (dir << 11);
    itd->BufferPointer[2] = (page + 8192) | ep->mult;
    uint8_t const slot = req->scheduled_uframe & 7;
    itd->xact[slot].offset = buffer & 0xfff;
    itd->xact[slot].length = req->buflen;
    itd->xact[slot].int_on_complete = 1;
  } else {
    ehci_sitd_t* sitd = &td->sitd;
    sitd->dev_addr = ep->daddr;
    sitd->ep_number = tu_edpt_number(ep->ep_addr);
    sitd->hub_addr = ep->hub_addr;
    sitd->port_number = ep->hub_port;
    sitd->direction = dir;
    sitd->buffer[0] = buffer;
    sitd->buffer[1] = page + 4096;
    sitd->back.terminate = 1;
    uint8_t const count = (uint8_t) tu_max32(1, (req->buflen + 187u) / 188u);
    if (dir == TUSB_DIR_IN) {
      // Leave the first two start-split slots for common FS audio OUT packets.
      // Keep the remaining complete-split opportunities for IN data/retries.
      sitd->int_smask = 4;
      sitd->fl_int_cmask = 0xf0;
    } else {
      sitd->int_smask = (uint8_t) ((1u << count) - 1u);
      sitd->buffer[1] |= count | (count > 1 ? TU_BIT(3) : 0); // T-count, TP=Begin/All
    }
    if (ehci_data.regs->portsc_bm.nxp_port_speed != TUSB_SPEED_HIGH) {
      sitd->int_smask = sitd->fl_int_cmask = 0;
      sitd->buffer[1] = page + 4096;
    }
    sitd->total_bytes = req->buflen;
    sitd->int_on_complete = 1;
  }
...
static bool iso_xfer(uint8_t rhport, iso_ep_t* ep, uint8_t* buffer, uint16_t buflen) {
  ...
  if ((int32_t) (scheduled - earliest) < 0) {
    ...
    scheduled += ((behind + ep->interval - 1) / ep->interval) * ep->interval;
  }
  if (ep->speed == TUSB_SPEED_FULL) {
  ...
  for (size_t i = 0; i < CFG_TUH_EHCI_ISO_EP_MAX; i++) {
    ...
    for (size_t retired = 0; retired < CFG_TUH_XFER_QUEUE_DEPTH && ep->count; retired++) {
      ...
      if (req->armed) {
        ...
        if (ep->speed == TUSB_SPEED_HIGH) {
          uint8_t const slot = req->scheduled_uframe & 7;
          active = td->itd.xact[slot].active;
          error = td->itd.xact[slot].error || td->itd.xact[slot].babble_err || td->itd.xact[slot].buffer_err;
          actual = tu_edpt_dir(ep->ep_addr) ? td->itd.xact[slot].length : req->buflen;
        } else {
          active = td->sitd.active;
          error = td->sitd.error || td->sitd.buffer_err || td->sitd.babble_err ||
                  td->sitd.xact_err || td->sitd.missed_uframe;
          actual = req->buflen - tu_min16(req->buflen, td->sitd.total_bytes);
        }
      }
...
static bool iso_ep_close(uint8_t rhport, iso_ep_t* ep) {
  ...
  for (size_t slot = 0; slot < CFG_TUH_XFER_QUEUE_DEPTH; slot++) {
    for (size_t frame = 0; frame < FRAMELIST_SIZE; frame++) {
      ehci_link_t* link = &ehci_data.period_framelist[frame];
      iso_td_t* td = &ehci_data.iso_td[index][slot][frame];
      ...
      if (!link->terminate) {
        *link = td->itd.next;
        // Frame list is software-owned; a predecessor descriptor was invalidated
        // above while DMA was stopped, preserving its hardware-written status.
...
void qhd_xfer_complete_isr(ehci_qhd_t * qhd) {
  ...
  if ( !qtd_overlay->active && qhd->attached_qtd != NULL ) {
    ...
    hcd_dcache_invalidate(qhd->attached_qtd, sizeof(ehci_qtd_t));
    if (qhd->attached_qtd->active) {
      return;
    }
